...
class PrimalCutsForeQuartersController extends AdminController
{
    protected $title = 'Primal Cuts - Fore Quarters';
    ...
    public function index(Content $content)
    {
        return $content
            ->title($this->title)
            ->description('Fore quarter primal cuts breakdown')
            ->body($this->grid());
    }

    /**
     * Make a grid builder - Shows fore quarter cuts grouped by EID
     */
    protected function grid()
    {
        $grid = new Grid(new SlaughterRecord());

        // Only show records that have fore quarter cuts
        $grid->model()
            ->whereHas('distributions', function($q) {
                $q->where('source_address', 'like', 'Fore-%')
                  ->whereNotIn('source_address', ['Fore-1/4 - Right', 'Fore-1/4 - Left']);
            })
            ->orderBy('created_at', 'desc');

        // Header with EID
        $grid->column('e_id', 'EID')->expand(function ($model) {
            // Get all fore quarter cuts for this carcass
            $cuts = SlaughterDistributionRecord::where('source_id', $model->id)
                ->where('source_address', 'like', 'Fore-%')
                ->whereNotIn('source_address', ['Fore-1/4 - Right', 'Fore-1/4 - Left'])
                ->get();
            ...
            foreach($cuts as $cut) {
                ...
                // Format: "Fore-CutName - Side" or similar
                ...
                // Clean cut name (remove "Fore-" prefix)
                $cutName = str_replace('Fore-', '', $cutName);
            ...
            $html .= '<tr><th>Primal Cut</th><th>FL (Kg)</th><th>FR (Kg)</th><th>Total (Kg)</th></tr>';
            $html .= '</thead><tbody>';
        ...
        // Summary columns
        $grid->column('total_fore_cuts', 'Total Cuts')->display(function() {
        ...
        $grid->column('total_fore_weight', 'Total Weight (kg)')->display(function() {
